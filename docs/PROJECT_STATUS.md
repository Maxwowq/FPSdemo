...
从 `App::run()` 中提取 `Camera` 类，在不改变现有行为的前提下收拢摄像机状态与计算。

验收标准：

- `Camera` 管理位置、朝向、世界上方向、移动参数以及 yaw/pitch。
- `Camera` 负责移动、旋转和生成观察矩阵，但不依赖 GLFW、`Window` 或 OpenGL。
- `App::run()` 只负责读取输入并调用 `Camera`，现有移动与视角行为保持不变。

## 后续任务

1. 完善光标释放与重新捕获操作。
2. 加入可提供空间参照的简单 3D 场景。
3. 处理窗口宽高比变化。
